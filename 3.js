...
function checki (number, min, max){
    if (number > min && number < max){
        console.log(`Число ${number} попадает в диапазон от ${min} до ${max}`)
    } else {
        console.log(`Число ${number} не попадает в диапазон от ${min} до ${max}`)
    }
}
checki(number, min, max);

function checkili (number, min, max){
    if (!(number < min || number > max)){
    console.log(`Число ${number} попадает в диапазон от ${min} до ${max}`)
    } else {
    console.log(`Число ${number} не попадает в диапазон от ${min} до ${max}`)
...
(arrayNum([5, 4, 3, -3, -10, -1, 8, -20, 0], 3));
...
for (let i = 0; i < users.length; i++){
     key = users[i]
if (key.age > 15){ newArr.push(key.name)
}
}
console.log(newArr);
